<template>
  ...
  >
    <v-sheet tile width="100vw" height="6vh" color="grey lighten-3" class="d-flex align-center fixed-top">
      <v-btn outlined small class="ma-4">
        {{ usernameJa }}&nbsp;さん
      </v-btn>
              ...
              <v-row>
                ...
                >
                  <v-select
                    ...
                    :items="timeListStart"
                    label="開始時間*"
                    :disabled="isRegularHoliday(selectedDate)"
                  ></v-select>
                </v-col>
                <v-col
                  cols="12"
...
async function wait(t, predicate) { // eslint-disable-line no-unused-vars
...
async function sleep(t) { // eslint-disable-line no-unused-vars
  return await new Promise(r => {
    setTimeout(() => {
      r()
    }, t)
...
export default {
  ...
  async created () {
    ...
    this.eventBusOn('reload-shift', today => {
      if (today != undefined) {
        this.today = today
      } else {
        ...
        this.today = momented.format('yyyy-MM-DD')
      }
      this.events = []
  ...
  methods: {
    ...
    checkAppWasUpdated () {
      ...
      setTimeout(async () => {
        ...
        let hash1 = (await API.get(apiname, `/check_data_updated/${yyyymm}`, param)).hash
        let hash2 = (await API.get(apiname, `/check_data_updated/${yyyymmPrev}`, param)).hash
        let hash3 = (await API.get(apiname, `/check_data_updated/${yyyymmNext}`, param)).hash
    ...
    async reload () {
      ...
      this.eventBusEmit('show-info', { show: false })
    ...
    */
    isDataLocked (error) {
      try {
        return error.response.data.message === 'Date Locked!!'
      } catch (e) { // eslint-disable-line no-unused-vars
      }
      return false
    },
    isDataIsRegularHoliday (error) {
      try {
        return error.response.data.message === 'Date is RegularHoliday!!'
      } catch (e) { // eslint-disable-line no-unused-vars
      }
      return false
    },
    ...
    async fetchData (yyyymm, onSucceeded, onFailed, showLoading) {
      ...
      API.get((await this.apiNameForLoginUser()), `/fetch_data/${yyyymm}`, { headers: (await this.getAuthHeader()) }).then(response => { // eslint-disable-line no-unused-vars
    ...
    async consumeRequest () {
      ...
      if (aRequest != undefined) {
        ...
        API.post((await this.apiNameForLoginUser()), `/${requestPath}`, params).then(response => { // eslint-disable-line no-unused-vars
          // this.eventBusEmit('show-debug', { show: true, message: [`prev: ${response.prev_hash.substring(0, 8)} / now: ${nowHash != undefined ? nowHash.substring(0, 8) : ""}`] })
          ...
          let errorMessage = `赤色の箇所をやり直してください。`
          if (this.isDataLocked(error)) {
            this.isLocked = true
            errorMessage = `データがロックされています。`
          }
          else
          if (this.isDataIsRegularHoliday(error)) {
            errorMessage = `定休日のためシフトを登録できません。`
          }
          ...
          this.eventBusEmit('show-error', { show: true, message: [ `保存に失敗しました ！!`, errorMessage ] })
    ...
    async onChange(info) {
      ...
      setTimeout(async () => {
        let loginUsername = await this.getUsername()
        //this.drawHolyday()
        this.showCalendar = true
        ...
        API.get((await this.apiNameForLoginUser()), `/fetch_username/${loginUsername}`, { headers: (await this.getAuthHeader()) }).then(response => { // eslint-disable-line no-unused-vars
          this.usernameJa = response.username
        }).catch((error) => { // eslint-disable-line no-unused-vars
          console.log('error', error)
        })
