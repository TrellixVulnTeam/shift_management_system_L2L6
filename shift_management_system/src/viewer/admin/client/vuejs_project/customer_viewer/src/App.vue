  ...
  created: function () {
    ...
    const checkAppWasUpdated = function () {
      ...
      API.get(process.env.VUE_APP_API_NAME, '/app_hash', { queryStringParameters: { origin: location.href } }).then(response => {
        self.eventBusEmit('show-debug', { show: true, message: [`app_hash: ${response.hash.substring(0, 8)} / version: 1.0`] })
        console.log(response)
        console.log(`CHECK_XXXXXXXX__XXX ${self.lsget('app_hash')} / ${response.hash}`);
